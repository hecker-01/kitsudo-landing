...
Terminal / hacker aesthetic — monospace type, typewriter reveals, a boot-log
loader, `$ command` headings, bracketed `[ BUTTONS ]` — re-skinned to Kitsudo's
**Catppuccin Mocha** palette with Mauve / Lavender accents and the fox mascot.

## Stack

- **Vite 8** + **Vue 3** `<script setup>` SFCs (plain JS)
- **Vue Router 4/5** (history mode) — `/`, `/privacy`, `/logs`, `*` → 404
...
- **Logo** — replace `public/logo.svg` (hero/favicon) and add `public/logo.png`
  (social meta). Keep the same filenames — no code changes needed.
...
- **Theme tokens** — Catppuccin Mocha colors / fonts / animations live in the
