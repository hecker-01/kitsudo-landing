...
const { output, isComplete, run } = useTypewriter(props.cmd, {
    delay: 55,
    start: false,
});
...
<template>
    <div ref="target" class="mb-10">
        <h2 class="font-mono text-2xl font-bold sm:text-3xl">
            ...
            ><span class="text-on-surface">{{ output }}</span
            ><span
        ...
        <p
            v-if="subtitle"
            class="mt-2 max-w-2xl text-sm text-on-surface-muted sm:text-base"
        >
            {{ subtitle }}
        </p>
    </div>
